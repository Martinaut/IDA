...
public class ResolveOperationInput extends BaseAction {
    ...
    protected void execute(ActionExecutionContext ctx, ContextModel ctxModel) throws ModelException, SCXMLExpressionException {
        logger.info("Executing action 'ResolveOperationInput'.");

        // Get Data
        String input = ctxModel.getUserInput();

        // Resolve Data
        ResolveOperationInputModel model = new ResolveOperationInputModel(
                getCurrentState(),
        ...
        String event = new OperationInputResolver().resolveOperationInput(model);
